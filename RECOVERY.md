...
**Pending:** Two post-sign-off tasks (no gates left): (1) **commit** the navbar-fix + C6 docs to `phase2.2-admin-portal-1` — feature-branch checkpoints, NEVER a bundled sweep to `main` (the new hard gate); (2) **promote** the 2 structural lessons (Gate-M collapse-vs-desktop-frame split · commit-checkpoint hard gate) to the central FFM playbook — operator confirming the exact file before any write. Parked: DockBloxx linting pass.

**Next step:** Commit the changes (2 checkpoints), then the central-playbook promotion (awaiting operator's file pick). Post-FFM: DockBloxx linting (parked — memory `post-c6-linting`).

**Files in flight (uncommitted since `7cd9b02`):** `src/components/global/Navbar.tsx` (dismiss + Radix-popper guard + theme-pick close) · `src/components/global/ThemeToggler.tsx` (optional `onSelect`) · `src/__tests__/global/Navbar.mobile.test.tsx` (7 tests) · `RECOVERY.md` · new untracked `playbook/RETROSPECTIVES/RUN_001_LESSONS.md`.
