...
**Last action (2026-06-26, latest):** Removed vestigial `(members)/members-portal` route group + orphaned `components/layout/Sidebar.tsx`. Salvaged the richer profile form into `/profile` with a **role-aware** identity badge (role fetched server-side in `profile/page.tsx` via `getUserRole`, passed as required prop; Admin/Superadmin→destructive, Member→success). Repointed member portal link `/members-portal`→`/owedbook` in `UserMenu`+`MobileNav`; test repointed + admin-badge case added. Docs synced (ROUTES_AND_SURFACES, ARCHITECTURE). **Triad: tsc 0 / jest 118·25 / build 0.** Then fixed the `/register` dead link: 3 "Start free trial" CTAs → `/auth?tab=register`, `AuthTabs` honors `?tab=` (Suspense-wrapped, `/auth` stays static). Triad still tsc 0 / jest 118·25 / build 0. Also renamed package `qr-next13-supabase-v1`→`cyber-pharma` (package.json + lockfile). No open flags. *Prior:*

**Earlier (2026-06-26):** `/docs` refresh + README index — see `session_2026-06-26.md` "PENDING_APPROVAL → DONE" block. 3 new app docs (PROJECT_OVERVIEW / APP_ARCHITECTURE / ROUTES_AND_SURFACES), README is now the `/docs` index, foundation docs de-staled (removed dead superadmin-portal refs, enum→`app-role.ts`), TESTING + MANUAL_TESTING rewritten to current reality. Docs-only — no `src/` change, suite still 117/25. Surfaced for later: legacy `(members)/members-portal` vestige + stale `package.json` name. *Prior:*

**Earlier last action:** Two post-FFM housekeeping passes on branch `linting-1` (both **tsc 0 / jest 117·25 / build 0**, lint **0 errors**): (1) **ESLint from zero** — Next 16 / ESLint 9 flat config (`eslint.config.mjs`, DockBloxx rules byte-faithful: no-unused-vars + no-explicit-any as warn, `^_` ignore added for frozen Phase-7 params), `"lint": "eslint ."` (next lint is gone in Next 16), fixed the 17 preset errors (no-require-imports→ESM imports + a `grid-auto-fit` type shim; empty-interface→type alias) and downgraded `react-hooks/set-state-in-effect`→warn (review-later, incl. known Navbar fetchUser); cleaned the safe unused-vars (incl. Logout now logs the real error), deleted dead dup `logout/route-1.ts`, added `scripts/lint-check.sh` (grouped manual lint summary), ignored `agent_docs/**`. (2) **Navigation spinner** — root-layout overlay (`NavigationSpinner`) shows `SpinnerLarge` on every primary-nav request across owedbook/admin/profile/moose, driven by `useLinkStatus` probes (desktop/sidebar) + direct `setPending` on mobile-menu + logo clicks (probe unmounts with the closing panel), min-display ~400ms (mock data is instant), cleared on `usePathname` change; `(public)/loading.tsx` upgraded to `SpinnerLarge` for redirect consistency.
