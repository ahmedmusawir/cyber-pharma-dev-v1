...
Last action: **Cluster 3 COMPLETE — OwedBook service wired to real mock data. PHASE 2 DONE (pending operator visual sign-off).** Extended `OwedBookRow` (operator-approved, DATA_CONTRACT v1.1): `new_paid`/`updated_difference` (Updated tab), real `aac`/`federal_expected`/`federal_diff` (Federal tab — now distinct from demo `medicaid_rate`/`owed`), `report_file`, nullable `pbm`. Placed 150 scrubbed fixture rows → `src/mocks/owedbook.ts` (camelCase placeholders renamed to contract snake_case; drug_name/bin dropped). Wired `owedBookService` (getKpis/getRows/getSummary/getPbmOptions) with date+PBM+status filters + pagination INSIDE the service; screen/UI calls unchanged. `columns.tsx` Federal tab uses real federal fields, Updated tab filled, Report column reads report_file. Triad: tsc 0 / jest **86·18** (+9 service tests verifying 7 PBMs / 150 scripts / pagination+clamp / per-tab scoping / filters / zero-result→empty / all 5 chips) / build 0. **G7 clean** (only the service imports the mock). NOTE: `agent_docs/CURRENT_APP/owedbook_mock_data.ts` staging file is now superseded by `src/mocks/owedbook.ts` — left in place (operator-supplied; recommend deleting). Live visual check is operator's (auth-gated).

Prior action: **Profile promoted to a role-aware navbar link (UI_SPEC v1.6).** Navbar now: ADMIN = OwedBook · Admin Portal · Profile; MEMBER = OwedBook · Profile (no empty navbar). Active via usePathname; same in mobile panel; Log out stays in avatar dropdown. Spec §E/§F updated + v1.6. `/profile` page untouched. Triad green (tsc 0 / jest 76·17 / build 0). UNCOMMITTED on `87b3382` (with the mobile-shell work). **Central factory docs also edited this session — see session log; operator lifting to central stack.**
